# Centralized warning + hardening flags. Apply with bencode_apply_warnings(<target>).
#
...
function(bencode_apply_warnings target)
    if(MSVC)
        set(_warnings
            /W4
            /permissive-
    ...
    if(NOT DEFINED BENCODE_FORTIFY_LEVEL_CACHED)
        set(CMAKE_REQUIRED_FLAGS "-O1 -Werror -D_FORTIFY_SOURCE=3")
        check_c_source_compiles("#include <string.h>
int main(void){char b[8]; strcpy(b,\"hi\"); return b[0];}" BENCODE_HAVE_FORTIFY3)
        unset(CMAKE_REQUIRED_FLAGS)
        if(BENCODE_HAVE_FORTIFY3)
            set(BENCODE_FORTIFY_LEVEL_CACHED 3 CACHE INTERNAL "")
        else()
            set(BENCODE_FORTIFY_LEVEL_CACHED 2 CACHE INTERNAL "")
        endif()
    endif()
    target_compile_definitions(${target} PRIVATE
        $<$<NOT:$<CONFIG:Debug>>:_FORTIFY_SOURCE=${BENCODE_FORTIFY_LEVEL_CACHED}>
    )
    ...
    if(NOT DEFINED BENCODE_HAVE_STRICT_FLEX_ARRAYS_CACHED)
        check_c_compiler_flag("-fstrict-flex-arrays=3" BENCODE_HAVE_STRICT_FLEX_ARRAYS)
        set(BENCODE_HAVE_STRICT_FLEX_ARRAYS_CACHED "${BENCODE_HAVE_STRICT_FLEX_ARRAYS}"
            CACHE INTERNAL "")
    endif()
    if(BENCODE_HAVE_STRICT_FLEX_ARRAYS_CACHED)
    ...
    if(NOT DEFINED BENCODE_HAVE_FILE_PREFIX_MAP_CACHED)
        check_c_compiler_flag("-ffile-prefix-map=/=/" BENCODE_HAVE_FILE_PREFIX_MAP)
        set(BENCODE_HAVE_FILE_PREFIX_MAP_CACHED "${BENCODE_HAVE_FILE_PREFIX_MAP}"
            CACHE INTERNAL "")
    endif()
    if(BENCODE_HAVE_FILE_PREFIX_MAP_CACHED)
        target_compile_options(${target} PRIVATE
